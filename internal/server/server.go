...
import (
	...
	"github.com/stockyard-dev/stockyard-mainspring/internal/store"
)
...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/mainspring/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("mainspring: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("mainspring: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"jobs": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/mainspring/")
		return
	}
	var e store.Job
...
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if patch.WebhookURL == "" {
		patch.WebhookURL = existing.WebhookURL
	}
	if patch.LastRunAt == "" {
